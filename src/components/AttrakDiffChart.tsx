...
export default function AttrakDiffChart({ answers, questions }: Props) {
  ...
  return (
    <div className="w-full flex flex-col gap-8 items-center">
      <Card className="w-full max-w-3xl p-6 shadow-lg" role="region" aria-label="Gráfico de descrição dos pares de palavras">
        <h3 className="font-bold text-xl mb-2" tabIndex={0}>Descrição dos Pares de Palavras</h3>
        <div className="mb-2 text-gray-700 text-sm">
          Pontuação média centralizada (negativo = avaliação ruim, positivo = avaliação boa)
        </div>
        <div className="sr-only" id="desc-grafico-pares">
          Gráfico de linhas mostrando a pontuação média de cada par de palavras do questionário AttrakDiff.
        </div>
        <ResponsiveContainer width="100%" height={Math.max(240, 32 * questions.length)} aria-labelledby="desc-grafico-pares">
          <LineChart
            data={wordPairData}
            layout="vertical"
      ...
      <Card className="w-full max-w-2xl p-6 shadow-lg" role="region" aria-label="Gráfico de médias PQ, HQ e ATT">
        <h3 className="font-bold text-xl mb-2" tabIndex={0}>Diagrama das Médias</h3>
        ...
        <div className="sr-only" id="desc-grafico-medias">
          Gráfico de linhas mostrando as médias dos fatores PQ (pragmático), HQ (hedônico) e ATT (atratividade).
        </div>
        <ResponsiveContainer width="100%" height={240} aria-labelledby="desc-grafico-medias">
      ...
      <Card className="w-full max-w-lg p-6 shadow-lg" role="region" aria-label="Matriz de Portfólio AttrakDiff">
        <h3 className="font-bold text-xl mb-2" tabIndex={0}>Matriz de Portfólio</h3>
        <div className="sr-only" id="desc-grafico-portfolio">
          Gráfico de dispersão mostrando a posição do resultado na matriz de portfólio do AttrakDiff, com eixos PQ e HQ.
        </div>
        <ResponsiveContainer width="100%" height={340} aria-labelledby="desc-grafico-portfolio">
